...
export default function App(){
 ...
 const court=useAsset("/assets/arena-3d-court-v1552.jpg?v=1552"),
 ...
 const[items,setItems]=useState(()=>{try{return JSON.parse(localStorage.getItem("jb1552-items"))||starter()}catch{return starter()}});
 ...
 const[scenes,setScenes]=useState(()=>{try{return JSON.parse(localStorage.getItem("jb1552-scenes"))||[]}catch{return[]}});
 ...
 useEffect(()=>{
  try{
   Object.keys(localStorage)
    .filter(k=>k.startsWith("jb") && !k.startsWith("jb1552-"))
 ...
 useEffect(()=>{
  try{
    const m=JSON.parse(localStorage.getItem("jb1552-meta")||"null");
    if(m){
 ...
 const save=()=>{localStorage.setItem("jb1552-items",JSON.stringify(items));localStorage.setItem("jb1552-scenes",JSON.stringify(scenes));localStorage.setItem("jb1552-meta",JSON.stringify({title,category,level,desc,fundamento}))};
 const exportPNG=()=>{
 ...
 const generatePlay=()=>{
   ...
   localStorage.setItem("jb1552-scenes",JSON.stringify(generated));
 ...
 const openStep=i=>{
   ...
   localStorage.setItem("jb1552-scenes",JSON.stringify(n));
 };
 const updateStep=()=>{
   if(!steps.length)return;
   const n=[...steps];n[stepIndex]={...n[stepIndex],items:clone(items)};
   setSteps(n);setScenes(n);localStorage.setItem("jb1552-scenes",JSON.stringify(n));flash(`Etapa ${stepIndex+1} atualizada ✓`);
 };
 const deleteStep=i=>{
   ...
   setItems(n[ni]?clone(n[ni].items):starter());localStorage.setItem("jb1552-scenes",JSON.stringify(n));
 };
 const sprite=i=>{
...
<div className="courtBox" ref={boxRef} style={{height:H*scale}}>
     <div style={{width:W*scale,height:H*scale}}>
      <Stage ref={stageRef} width={W*scale} height={H*scale} scaleX={scale} scaleY={scale} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={()=>{setDraft(null);setMode("select")}}>
       <Layer>
        {/* V1.55.2: UMA ÚNICA arena 3D realista.
            O JPG contém ambiente, areia, linhas oficiais e uma única rede vertical 3D.
            Jogadores, professor e elementos táticos continuam independentes. */}
